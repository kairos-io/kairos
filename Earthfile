...
docker:
    ...
    ELSE IF [ "$FLAVOR" = "debian" ] || [ "$FLAVOR" = "ubuntu" ] || [ "$FLAVOR" = "ubuntu-20-lts" ] || [ "$FLAVOR" = "ubuntu-22-lts" ] || [ "$FLAVOR" = "ubuntu-arm-rpi" ] || [ "$FLAVOR" = "ubuntu-20-lts-arm-rpi" ] || [ "$FLAVOR" = "ubuntu-22-lts-arm-rpi" ]
     RUN kernel=$(ls /boot/vmlinuz-* | head -n1) && \
            ln -sf "${kernel#/boot/}" /boot/vmlinuz
     RUN kernel=$(ls /lib/modules | head -n1) && \

...
By default `AuroraBoot` will automatically attempt to bootstrap other machines in the same network that are configured for booting from network, with no further configuration or setting necessary.
...
Note: the example below are implying a `config.yaml` cloud config file to be present in the current directory. 

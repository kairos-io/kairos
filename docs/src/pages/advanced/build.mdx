...
The Spec will create a pod that builds the ISO ( we can follow the process by tailing to the containers log ) and later makes it accessible to its own dedicated service (nodeport by default):
...
Custom user-data from the Cloud provider is supported, additionally the CRD allows to embed a custom cloudConfig additionally that we can use to make configuration permanent.
...
Will create a `hello-kairos.raw` file, which is an EFI bootable raw disk, bootable in QEMU and compatible with AWS:
...
To run locally with QEMU:

```bash
qemu-system-x86_64 -m 2048 -bios /usr/share/qemu/ovmf-x86_64.bin -drive if=virtio,media=disk,file=output.raw
```

Since the image come with only the `recovery` system populated, we need to apply a cloud-config similar to this one which tells which container image we want to deploy.
...
Follow the procedure described in [AWS docs](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/creating-an-ami-ebs.html#creating-launching-ami-from-snapshot) to register an AMI from snapshot. Used all default settings unless for the firmware, set to force to UEFI boot.

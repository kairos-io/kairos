...
5. **Application order**: Files in the OEM partition are applied in lexicographic (alphabetical) order by their final file name. This is independent of the order entries appear in the `oem_files` list. The order of application matters because standard cloud-config merge semantics apply: files processed later can extend or override settings from earlier files.
   ...
   - The names Kairos writes to the OEM partition itself are rejected outright, not just discouraged: `90_custom.yaml` through `99_custom.yaml` (the launch cloud-config, one file per `--cloud-config` source), `10_ssh_hardening.yaml`, `10_user_custom_mounts.yaml`, `10_extensions_ignore_signatures.yaml`, and `99_phonehome_remote.yaml`. An entry resolving to one of these fails validation before anything is written, instead of silently replacing the installed system's own configuration, including its users and `ssh_authorized_keys`.

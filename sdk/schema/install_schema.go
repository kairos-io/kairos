...
type InstallSchema struct {
	...
	OEMFiles               []OEMFileSchema     `json:"oem_files,omitempty" mapstructure:"oem_files" description:"Cloud-config files to write into the cloud-config directory of the installed system (/oem, or /usr/local/cloud-config on devices without an OEM partition) so they are applied on its first boot."`
